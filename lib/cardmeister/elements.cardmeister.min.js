(function () {
  ...
  let pars = (CE, init = {}) =>
    CE.getAttributeNames().reduce(
      (s, a) => ((s[a] = CE.getAttribute(a)), s),
      init
    );
  let props = (CE) =>
  ...
  function s({
    ...
    try {
      ...
      let j = l(v) ? e(v)[z] : v,
        ...
        S = (c, m, l, t, e, a) =>
          `<rect${D(c, m, l, t)} rx='${e}' ry='${e}' ${a}></rect>`,
      ...
      if (
        ...
        [0, 1, 2, 3].map(
          (c) => (K += Z(F + c, V(600), r[0][c], E(l(v) ? e(v)[z] : v)))
        ),
        ...
        b && (K += N(b, 0, 10, '', A, 42)),
          (K +=
            `<pattern patternUnits='userSpaceOnUse' id='A' ${D(
              w,
              w
            )}><path d='M${c} 0L${w} ${c}L${c} ${w}L0 ${c}Z' ${E(
              L
            )}></path></pattern>` + S(m, l, -m / 2, -l / 2, d, E('url(#A)')));
      }
      if (s > 10) {
      ...
      window.console.warn(
        'See https://github.com/cardmeister/cardmeister.github.io'
      );
    }
  }
  let r = [
